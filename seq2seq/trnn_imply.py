            ...
            if is_sample and time_step > 0: 
                with tf.variable_scope(tf.get_variable_scope(), reuse=True):
                    inp = tf.cond(tf.cast(samples[time_step], tf.bool),  lambda:tf.identity(inp) , \
                       ...
                       # lambda:fully_connected(cell_output, input_size))
...
def tensor_rnn_with_feed_prev_TP(cell, inputs, is_training, config, initial_states=None):
    ...
    is_sample = config.use_sched_samp
    ...
    with tf.variable_scope("trnn") as varscope:
        ...
        if initial_states is None:
            ...
            for lag in range(config.num_lags):
                # initial states: lag num, layer num, LSTM state tuple num
                # initial state: layer num, LSTM state tuple num
                initial_state =  cell.zero_state(batch_size, dtype= tf.float32)
                initial_states.append(initial_state)
                initial_p = cell.zero_state(batch_size, dtype= tf.float32)
        ...
        for time_step in range(num_steps):
            ...
            if is_sample and time_step > 0: 
            # is_sample: true in train decoders
                pdb.set_trace() # input fully connected layer test
                with tf.variable_scope(tf.get_variable_scope(), reuse=True):
                    inp = tf.cond(tf.cast(samples[time_step], tf.bool),  lambda:tf.identity(inp) , \
                       lambda:fully_connected(cell_output, input_size, activation_fn=tf.sigmoid))
            ...
            if not is_training and prev is not None and time_step >= inp_steps:
            # if is tested: use output of previous cell as input
            #               is_training = False & inp_steps = 0
                with tf.variable_scope(tf.get_variable_scope(), reuse=True):
                    inp = fully_connected(cell_output, input_size, activation_fn=tf.sigmoid) 

            states = _list_to_states(states_list)
            ps = _list_to_states(initial_ps)
            """input tensor is [batch_size, num_steps, input_size]"""
            """states: lag number * layer number * hidden state number"""
            (cell_output, state, ps)=cell(inp, states)
